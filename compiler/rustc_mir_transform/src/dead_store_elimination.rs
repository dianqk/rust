...
impl DeadStoreElimination {
    ...
    fn eliminate<'tcx>(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        let borrowed_locals = borrowed_locals(body);

        // If the user requests complete debuginfo, mark the locals that appear in it as live, so
        // we don't remove assignments to them.
        let mut always_live = self.debuginfo_non_ssa_locals(tcx, body);
    ...
    fn debuginfo_non_ssa_locals<'tcx>(
        &self,
        tcx: TyCtxt<'tcx>,
        body: &Body<'tcx>,
    ) -> DenseBitSet<Local> {
        ...
        if !matches!(self, DeadStoreElimination::Initial) {
            let typing_env = body.typing_env(tcx);
            let ssa = SsaLocals::new(tcx, body, typing_env);
            let mut non_ssa_locals: DenseBitSet<Local> =
